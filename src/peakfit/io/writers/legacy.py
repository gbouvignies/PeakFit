"""Legacy format output writer for backward compatibility."""
...
if TYPE_CHECKING:
    ...
    from peakfit.engine.results import ClusterEstimates, FitResults


# Pattern to match position parameters: F10, F20, x0, y0, etc.
_POSITION_PARAM_PATTERN = re.compile(r"(F\d+0|[xyza]0)$")
_MAX_PEAK_NAMES_IN_LABEL = 2
...
class LegacyWriter:
    ...
    def write_profiles(self, results: FitResults, output_dir: Path) -> None:
        ...
        for cluster in results.clusters:
            ...
            peak_amplitudes: dict[str, list[Any]] = {}
    ...
    def _write_peak_profile(
        ...
        with filepath.open("w") as f:
            ...
            # Parameters section
            self._write_legacy_params(f, cluster, peak_name, prec)
            ...
            for cluster in results.clusters:
                for peak_name in cluster.peak_names:
                    ...
                    position_params = [
                        ...
                        if peak_name in param.name and _POSITION_PARAM_PATTERN.search(param.name)
                    ]

                    if position_params:
                        # Sort by dimension
                        position_params.sort(key=self._shift_sort_key)
    ...
    def write_params(self, results: FitResults, filepath: Path) -> None:
        ...
        with filepath.open("w") as f:
            ...
            for cluster in results.clusters:
                peak_label = ", ".join(cluster.peak_names[:_MAX_PEAK_NAMES_IN_LABEL])
                if len(cluster.peak_names) > _MAX_PEAK_NAMES_IN_LABEL:
                    peak_label += "..."
    ...
    def _shift_sort_key(self, p: Any) -> tuple[int, int | str]:
        """Sort position parameters by dimension label."""
        match = _POSITION_PARAM_PATTERN.search(p.name)
        if match:
            dim = match.group(1)
            if dim.startswith("F"):
                # F10 -> (0, 1), F20 -> (0, 2)
    ...
    def _write_legacy_params(
        self, f: Any, cluster: ClusterEstimates, peak_name: str, prec: int
    ) -> None:
        """Write legacy parameter block."""
        f.write("# Parameters:\n")
