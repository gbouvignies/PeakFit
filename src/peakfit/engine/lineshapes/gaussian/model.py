...
def param_specs(
    ...
    """Return default parameter specs for a Gaussian singlet."""
...
def function(
    ...
    *,
    ...
    """Evaluate Gaussian singlet on a grid.

    Parameters must be arrays with one value per peak.
    """
    grid = require_grid(context, shape="Gaussian singlet")
    ...
    values = sign * kernel(dw_hz, lw_arr)
    return values
...
@register_shape(NAME)
class Gaussian(SimpleSingletBase):
    """Gaussian singlet lineshape: G(Δω) = exp(-c × Δω²)."""
...
def param_specs_doublet(
    ...
    """Return default parameter specs for a Gaussian doublet."""
    pos_bounds_ppm = estimate_cs_bounds_ppm(context)
...
def function_doublet(
    ...
    *,
    ...
    """Evaluate Gaussian doublet on a grid.

    Parameters must be arrays with one value per peak.
    """
    grid = require_grid(context, shape="Gaussian doublet")
    x_arr = np.atleast_1d(np.asarray(x, dtype=np.float64))
    cs_arr = np.atleast_1d(np.asarray(cs, dtype=np.float64))
    lw_arr = np.atleast_1d(np.asarray(lw, dtype=np.float64))
...
class GaussianDoublet(SimpleDoubletBase):
    """Gaussian doublet lineshape with J-coupling."""
