...
class PlotService:
    ...
    def generate_intensity_plots(
        ...
        """Generate intensity profile plots from fit summary."""
        ...
        return self._generate_paginated_plots(
    ...
    def generate_cest_plots(
        ...
        def _prepare_data(points: list[tuple[float, float, float]]) -> Any | None:
            return self._prepare_cest_data(points, ref_points)
    ...
    def generate_cpmg_plots(
        ...
        def _prepare_data(points: list[tuple[float, float, float]]) -> Any:
            return self._prepare_cpmg_data(points, time_t2)
    ...
    def _generate_paginated_plots(
        ...
        """Generate paginated plots from results (generic)."""
        # Load Results
        ...
        for cluster in summary.clusters:
            for amp in cluster.amplitudes:
                x_val = amp.z_value if amp.z_value is not None else float(amp.plane_index)
                std_error = amp.std_error if amp.std_error is not None else 0.0
                peak_data[amp.peak_name].append((x_val, amp.value, std_error))
        if not peak_data:
            peak_data = self._load_peak_data_from_intensities_csv(results_dir)
        ...
        with PdfPages(output_path) as pdf:
            for peak in sorted_peaks:
                try:
                    ...
                    self._reporter.warning(f"Failed to plot {peak}: {e}")
    ...
    def _load_peak_data_from_intensities_csv(
        ...
        """Load per-peak series from parameters/intensities.csv.

        This is used as a fallback when amplitudes are omitted from fit_summary.json.
        """
        peak_data: defaultdict[str, list[tuple[float, float, float]]] = defaultdict(list)
        intensities_path = self._resolve_intensities_csv(results_dir)
        ...
        with intensities_path.open(encoding="utf-8", newline="") as handle:
            ...
            for row in reader:
                ...
                intensity = self._parse_float(row.get("intensity"), default=np.nan)
                if not np.isfinite(intensity):
                    continue

                plane_index = self._parse_float(row.get("plane_index"), default=0.0)
                z_value = self._parse_float(row.get("z_value"), default=np.nan)
                x_val = z_value if np.isfinite(z_value) else plane_index

                std_error = self._parse_float(row.get("intensity_err"), default=0.0)
    ...
    def _resolve_intensities_csv(results_dir: Path) -> Path | None:
        ...
        candidates = (
            results_dir / "parameters" / "intensities.csv",
            results_dir.parent / "parameters" / "intensities.csv",
        )
    ...
    def _prepare_cest_data(
        self, points: list[tuple[float, float, float]], ref_points: list[int]
    ...
    def _prepare_cpmg_data(
        self, points: list[tuple[float, float, float]], time_t2: float
    ) -> tuple[Any, Any, Any]:
    ...
    def generate_mcmc_diagnostics(
        ...
        with PdfPages(output_path) as pdf:
            for (
                ...
                chains, parameter_names = self._select_mcmc_plot_parameters(
                    raw_chains, raw_parameter_names
                )
                ...
                fig_trace.suptitle(
                    f"Cluster {cluster_id}: Trace Plots", fontsize=14, fontweight="bold"
                )
    ...
    @staticmethod
    def _select_mcmc_plot_parameters(
        chains: Any, parameter_names: list[str]
    ) -> tuple[Any, list[str]]:
        ...
        amp_indices = [
            i for i, name in enumerate(names) if PlotService._is_amplitude_parameter_name(name)
        ]
