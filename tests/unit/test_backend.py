...
class TestGetBestBackend:
    ...
    def test_prefers_jax_if_available(self):
        """Test that JAX is preferred if available."""
        available = get_available_backends()
        best = get_best_backend()

        # Priority: JAX > Numba > NumPy
        if "jax" in available:
            assert best == "jax"
...
class TestBackendFunctions:
    ...
    def test_backend_functions_work_after_switch(self):
        ...
        if "jax" in get_available_backends():
            ...
            # Convert to numpy if needed (JAX arrays)
            result_jax_np = np.asarray(result_jax)
            assert isinstance(result_jax_np, np.ndarray)
            assert len(result_jax_np) == 2
            # Results should be approximately the same
            assert np.allclose(result_numpy, result_jax_np)
