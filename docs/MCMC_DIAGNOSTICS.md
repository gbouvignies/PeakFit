...
peakfit mcmc Fits/ --walkers 64 --steps 2000

# Manual burn-in override (disables adaptive determination)
peakfit mcmc Fits/ --walkers 64 --steps 2000 --burn-in 500

# Disable adaptive burn-in (uses default 200 steps)
peakfit mcmc Fits/ --no-auto-burnin

# Analyze specific peaks only
peakfit mcmc Fits/ --peaks 2N-H --peaks 5L-H
```

This command will:

- Sample the posterior distribution using MCMC
- **Automatically determine burn-in** using R-hat convergence monitoring
- Compute R-hat and ESS diagnostics
- Display convergence statistics in the console
...
peakfit plot mcmc Fits/

# Custom output file
peakfit plot mcmc Fits/ --output my_diagnostics.pdf
```
...
**Note:** `peakfit mcmc` saves per-cluster chain files by default under
`Fits/<timestamp>/chains/cluster_*_chains.h5`, and `peakfit plot mcmc` reads
...
```bash
peakfit mcmc Fits/ --walkers 32 --steps 500
```
...
```bash
peakfit mcmc Fits/ --walkers 32 --steps 2000
```
...
```bash
peakfit mcmc Fits/ --walkers 64 --steps 5000
```
...
```bash
peakfit mcmc Fits/ --walkers 128 --steps 10000
```
...
peakfit mcmc Fits/ --walkers 64 --steps 5000 --burn-in 1000
...
1. **Kruschke, J. K. (2021)**. Bayesian analysis reporting guidelines. _Nature Human Behaviour_, 5(10), 1282-1291. https://doi.org/10.1038/s41562-021-01177-7

2. **Gelman, A., & Rubin, D. B. (1992)**. Inference from iterative simulation using multiple sequences. _Statistical Science_, 7(4), 457-472.

3. **Vehtari, A., et al. (2021)**. Rank-normalization, folding, and localization: An improved R-hat for assessing convergence of MCMC. _Bayesian Analysis_, 16(2), 667-718.
